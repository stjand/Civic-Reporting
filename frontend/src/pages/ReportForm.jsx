// File: ReportForm.jsx (FIXED for React state warning)
...
const MapPicker = ({ initialLocation, onLocationSelect, isFullscreen = false }) => {
	...
	const handleLocationSelect = async (latlng) => {
		// 🟢 FIX: Call setPosition immediately to update the marker before geocoding
		setPosition(latlng);
		setLoading(true);
		try {
			// Call the local backend proxy endpoint
			const response = await fetch(
				`/api/geocode/reverse?lat=${latlng.lat}&lon=${latlng.lng}`
			);
			...
			// Check for success flag from our own proxy response (from index.js)
			if (!data.success) {
				throw new Error(data.error || 'Address lookup failed on server.');
			}
			
			// The proxy now returns the address under the 'address' key
			onLocationSelect(latlng, data.address || 'Address not found'); 
		} catch (error) {
			console.error('Geocoding error:', error);
			// Fallback for address lookup failure
			onLocationSelect(latlng, 'Address lookup failed. Coordinates recorded.');
		} finally {
			setLoading(false);
		}
	};
...
// Progress Stepper Component (Unchanged)
...
const ModernReportForm = () => {
	...
	// ModernReportForm Component (Inside reverseGeocode function)
	const reverseGeocode = useCallback(async (latlng) => {
			try {
				...
				// Check for success flag from our own proxy response
				if (!data.success) {
						throw new Error(data.error || 'Address lookup failed on server.');
				}
				...
				return 'Address lookup failed. Coordinates recorded.';
			}
	}, []);
	...
	const handleSubmit = async () => {
		...
		formData.photos.forEach((photoObj) => {
			...
			// Use the original file object for better type inference on the server
			submitData.append(`photos`, photoObj.file, photoObj.file.name);
		});
		...
		try {
			// Endpoint is '/reports' as the apiClient already prepends the base URL and '/api'
	...
	// 🟢 FIX 6: The navigation logic is already in useEffect. 
	// The problem is that navigating immediately in a render cycle causes the warning. 
	// The existing useEffect correctly handles this by running *after* render. 
	// The logic itself is sound (timer + navigate). We can remove the hidden button 
	// as it seems to be an old artifact and the timer should handle the navigation.
	useEffect(() => {
		if (submitted) {
			const timer = setInterval(() => {
				setCountdown(prev => {
					if (prev <= 1) {
						clearInterval(timer);
						// 🟢 FIX: Ensure navigate is called after cleanup
						setTimeout(() => navigate('/citizen'), 0); 
						return 0;
					}
					return prev - 1;
				});
		...
		return (
			<div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 flex items-center justify-center p-4">
				<div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 text-center animate-fadeIn">
					...
					{/* 🟢 REMOVE: Hidden button that served no clear purpose and could be part of the confusion */}
