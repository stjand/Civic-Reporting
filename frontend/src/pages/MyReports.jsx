...
// 🟢 FIX 1: Import the API_BASE_URL to correctly build media paths
...
const MyReports = () => {
	...
	const fetchData = async () => {
		try {
			setLoading(true);
			// API service structure is { success: true, reports: [...] }
	...
	return (
		<div className="min-h-screen bg-slate-50 flex flex-col">
				...
				{sortedReports.length === 0 ? (
					...
					<div className="space-y-4">
						{sortedReports.map((report) => {
							...
							// 🟢 FIX 2: Create the full, correct URL for image and audio media in the list view
								...
								>
									<div className="flex flex-col sm:flex-row">
										...
										<div className="w-full sm:w-48 h-48 sm:h-auto bg-slate-100 flex-shrink-0 relative">
											{report.photo_url ? (
												<img 
													src={fullPhotoUrl} // 🟢 FIX: Use fullPhotoUrl
			...
			{/* Report Details Modal - UPDATED SECTION */}
			{selectedReport && (
					...
					<div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
						...
						<div className="sticky top-0 bg-white border-b border-slate-200 px-4 sm:px-6 py-4 flex items-center justify-between z-10">
							<div className="flex-1 min-w-0 mr-4">
								...
								<p className="text-sm text-slate-500 font-mono flex items-center space-x-1">
						...
						<div className="flex-1 overflow-y-auto">
							...
							{selectedReport.photo_url && (
								<div className="w-full bg-slate-100">
									<img 
										src={`${API_BASE_URL}${selectedReport.photo_url}`} // 🟢 FIX 3: Use full URL in modal
							...
							<div className="p-4 sm:p-6 space-y-6">
								...
								<div>
									...
									<p className="text-slate-700 leading-relaxed">{selectedReport.description || 'No description provided.'}</p>
								</div>
								
								{/* *** NEW SECTION: ADMINISTRATIVE/AUDIT DETAILS *** */}
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100">
									{/* Submitted By */}
									<div>
										<label className="block text-xs font-semibold text-slate-500 uppercase mb-2">Submitted By</label>
										<div className="flex items-center space-x-2 text-slate-900">
											<User className="w-4 h-4 text-slate-400" />
											{/* NOTE: user_id is a UUID, showing it is not user-friendly. Assuming backend doesn't send a name here. */}
											<span className="text-sm font-medium">Citizen (Self-Reported)</span>
										</div>
									</div>

									{/* Last Updated By */}
									{selectedReport.updated_by_user_id && (
										<div>
											<label className="block text-xs font-semibold text-slate-500 uppercase mb-2">Updated By</label>
											<div className="flex items-center space-x-2 text-slate-900">
												<UserCheck className="w-4 h-4 text-slate-400" />
												<span className="text-sm font-medium">{selectedReport.updated_by_user_id}</span>
											</div>
										</div>
									)}
								</div>
								...
								{selectedReport.audio_url && (
									<div>
										...
										<div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
											<audio 
												controls 
												src={`${API_BASE_URL}${selectedReport.audio_url}`} // 🟢 FIX 4: Use full URL for audio
												className="w-full" 
											/>
										</div>
									</div>
										...
										<div className="flex items-start space-x-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
											...
											<div className="flex-1 min-w-0">
												<p className="text-sm font-medium text-slate-900 mb-1">
													{parseFloat(selectedReport.latitude)?.toFixed(6) || 'N/A'}, {parseFloat(selectedReport.longitude)?.toFixed(6) || 'N/A'}
												</p>
												<a
													// 🟢 FIX 5: Corrected Google Maps URL
